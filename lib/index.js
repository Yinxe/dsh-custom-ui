/**
 * @dshp-inx/theme-gallery host half —— 标准 DSH Bundle 插件包。
 ...
 * 这一环：
 *   - 注册 `dshp-inx-theme-gallery` settings 命名空间（标准 settings 存储），
 *     字段 themeId 持久化用户选中的主题 id；
 *   - 同源路由 GET /state 读、POST /theme 写，供 client 半在启动时恢复、点击时保存。
 *
 * 路由（全部同源校验）：
 *   GET  /ext/dshp-inx-theme-gallery/state   → { ok, themeId }
 *   POST /ext/dshp-inx-theme-gallery/theme   { themeId } → { ok, themeId }
 *
 * 持久化（对齐本地插件规范：settings.yaml 顶层 `dshp-inx-theme-gallery` 命名空间）。
 */

import z from '@deepseek-ai/schemastery'
import { settingsNamespace, installSettingsSection } from '@deepseek-ai/dsh-settings'

export const name = '@dshp-inx-theme-gallery'
export const inject = ['webServer']

/* ── 官方 settings 命名空间与 schema（settings.yaml: dshp-inx-theme-gallery）── */
export const NS = settingsNamespace('dshp-inx-theme-gallery')
...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/ext/dshp-inx-theme-gallery/state',
  ...
  }), 'dshp-inx-theme-gallery: state route')

  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/ext/dshp-inx-theme-gallery/theme',
    handler: async (req, res) => {
      if (!sameOrigin(req)) return json(res, 403, { ok: false, error: 'forbidden' })
  ...
  }), 'dshp-inx-theme-gallery: theme route')
}
